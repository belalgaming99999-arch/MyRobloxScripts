...
-- ========== SPEED DISPLAY (عداد السرعة فوق الرأس) ==========
local speedBillboard = nil
local speedLabel = nil

local function createSpeedDisplay()
    if speedBillboard then speedBillboard:Destroy() end
    speedBillboard = Instance.new("BillboardGui")
    speedBillboard.Name = "SpeedDisplay"
    speedBillboard.Size = UDim2.new(0, 100, 0, 30)
    speedBillboard.StudsOffset = Vector3.new(0, 3.5, 0)
    speedBillboard.AlwaysOnTop = true
    speedBillboard.ResetOnSpawn = false
    
    speedLabel = Instance.new("TextLabel", speedBillboard)
    speedLabel.Size = UDim2.new(1, 0, 1, 0)
    speedLabel.BackgroundTransparency = 1
    speedLabel.Text = "Speed: 0.0"
    speedLabel.TextColor3 = CrystalPurple
...
task.spawn(function()
    while true do
        pcall(function()
            local char = Player.Character
            if char and speedLabel then
                local hrp = char:FindFirstChild("HumanoidRootPart")
                if hrp then
                    local velocity = hrp.AssemblyLinearVelocity
                    local speed = math.floor((velocity.X^2 + velocity.Z^2)^0.5 * 10) / 10
...
-- ========== STEAL NEAREST (مع دائرة بنفسجية صغيرة) ==========
...
local function startLeftSteal()
    ...
    leftPathActive = true
    task.spawn(function()
        while leftPathActive and Toggles.LeftSteal do
            runLeftPath()
            task.wait(0.1)
        end
    end)
...
local function startRightSteal()
    ...
    task.spawn(function()
        while rightPathActive and Toggles.RightSteal do
            runRightPath()
            task.wait(0.1)
        end
    end)
...
SideMenu.Size = UDim2.new(0, 160, 0, 460)
...
local function CreateBtn(txt, parent, size, toggleKey)
    ...
    btn.MouseButton1Click:Connect(function()
        ...
        elseif toggleKey == "StealNearest" then
            StealNearestState = Toggles.StealNearest
            if StealNearestState then startStealNearest() else stopStealNearest() end
        elseif toggleKey == "AutoMedusa" then
            -- handled in loop
        elseif toggleKey == "LeftSteal" then
            if Toggles.LeftSteal then startLeftSteal() else stopLeftSteal() end
        elseif toggleKey == "RightSteal" then
            if Toggles.RightSteal then startRightSteal() else stopRightSteal() end
...
-- الأزرار (نفس الترتيب اللي انت طلبت)
...
-- أزرار LEFT و RIGHT (تحت كل حاجة زي الصورة)
local LR_Row = Row(SideMenu)
CreateBtn("LEFT", LR_Row, UDim2.new(0, 66, 1, 0), "LeftSteal")
CreateBtn("RIGHT", LR_Row, UDim2.new(0, 66, 1, 0), "RightSteal")
